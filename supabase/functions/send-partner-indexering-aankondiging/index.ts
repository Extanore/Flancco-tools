...
// ─────────────────────────────────────────────────────────────────────────────
// Rate limiting (in-memory; per Deno-isolate). 10 calls/min per IP. Caller is
// normaal pg_cron of admin-UI; rate-limit beschermt tegen accidental loops.
// ─────────────────────────────────────────────────────────────────────────────
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // 1) Planned-rij ophalen + sanity-checks
  ...
  if (!planned.aangekondigd_op) {
    // Veiligheidsklep: zonder admin-aankondiging-flag mag deze functie geen mails sturen.
    return jsonResp(200, { error: "not_announced_yet", skipped: true });
  }

  // 2) Partners ophalen — alle actieve, exclusief Flancco zelf
  ...
  // 3) Mails per partner versturen (parallel via Promise.allSettled)
...
function formatPct(p: number | null | undefined): string {
  ...
  // Toon zonder onnodige trailing zeroes, max 2 decimalen.
