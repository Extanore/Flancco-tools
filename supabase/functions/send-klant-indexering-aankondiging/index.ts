...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // 1) Contract + partner + index ophalen
  ...
  // 2) Partner ophalen voor branding
  ...
  // 4) Bereken capped pct + preview nieuwe prijzen
...
// ─────────────────────────────────────────────────────────────────────────────
// Finalize: UPDATE/INSERT contract_indexering_announcements + return response
// ─────────────────────────────────────────────────────────────────────────────
...
async function finalize(
  ...
  // Probeer UPDATE op pre-existing rij; als geen rij → INSERT (admin direct-call).
  if (geplandVoorDatum) {
    try {
      ...
      // Een DB-failure mag de mail-verzending niet ongedaan maken — log & continue.
  ...
  // Errors → 200 met skipped=true zodat retry-cron geen 5xx ziet.
  return jsonResp(200, {
    ok: status === "sent",
    status,
    reason: reason ?? undefined,
...
// ─────────────────────────────────────────────────────────────────────────────
// Email template (NL, partner-branded)
// ─────────────────────────────────────────────────────────────────────────────
